...
```mermaid
flowchart TD
    OGP["OGP — Online Gaming Platform"]
    OGP --> AUTH["Authentication & User Management\n/api/v1/auth"]
    OGP --> TM["Tournament Management\n/api/v1/tournaments"]
    OGP --> MS["Match & Streaming\n/api/v1/matches"]
    OGP --> LS["Leaderboard & Scoring\n/api/v1/leaderboard"]
    OGP --> PW["Payments & Wallet\n/api/v1/payments"]
    OGP --> AD["Organiser Admin Dashboard\n(UI layer)"]

    AUTH --> A1["Email OTP registration"]
    AUTH --> A2["Phone/password login + JWT"]
    AUTH --> A3["Profile picture upload"]

    TM --> T1["Create / list tournaments"]
    TM --> T2["Individual + squad registration"]
    TM --> T3["Auto-group solo pool into squads"]
    TM --> T4["Start: bracket or multi-lobby"]

    MS --> M1["Stream URL submission"]
    MS --> M2["Proof screenshot upload"]
    MS --> M3["Result / BR stats entry"]

    LS --> L1["Per-tournament points"]
    LS --> L2["Organiser-set scoring"]
    LS --> L3["Global career leaderboard"]

    PW --> P1["eSewa / Khalti entry fees"]
    PW --> P2["Server-side verification"]
    PW --> P3["Wallet + prize credit + withdrawals"]

    AD --> D1["Create / monitor / verify"]
    AD --> D2["Registered participants view"]
    AD --> D3["Player statistics report"]
```
...
```mermaid
flowchart LR
    R[Research] --> B[Product Backlog]
    B --> U[UML & Diagrams]
    U --> F[Supervisor Meeting & Feedback]
    F --> W[Wireframes & UI/UX]
    W --> D[Development]
    D --> G[Git Push]
    G --> T[Testing]
    T --> BR[Bug Report]
...
```mermaid
gantt
    title OGP Project Roadmap
    dateFormat  YYYY-MM-DD
    axisFormat  %b %d

    section Inception
    Proposal & problem definition      :a1, 2024-11-04, 7d
    Requirements & PRD                  :a2, after a1, 7d
    Literature review                  :a3, after a1, 10d

    section Design
    Architecture & data model          :d1, after a2, 5d
    UML, ERD, wireframes                :d2, after d1, 7d

    section Core MVP Build
    Auth & User Management             :s1, after d2, 5d
    Tournament Management              :s2, after s1, 6d
    Match & Streaming                  :s3, after s2, 5d
    Payments & Wallet                  :s4, after s3, 6d
    Leaderboard & Scoring              :s5, after s4, 4d
    Admin Dashboard                    :s6, after s5, 5d

    section Enhancement Phase
    Global career leaderboard          :e1, after s6, 3d
    Organiser-set scoring              :e2, after e1, 2d
    Participant visibility             :e3, after e2, 2d
    Auto-group & multi-lobby           :e4, after e3, 5d

    section Finalisation
    Integration & end-to-end testing   :f1, after e4, 6d
    Bug fixing & hardening             :f2, after f1, 5d
    Documentation & report             :f3, after f1, 12d
    Submission                         :milestone, f4, after f3, 0d
```
...
```mermaid
erDiagram
    USER ||--o{ TOURNAMENT : organises
    USER ||--o| WALLET : owns
    USER }o--o{ TOURNAMENT : "registered in (solo / pool)"
    TOURNAMENT ||--o{ TEAM : "has squads"
    USER ||--o{ TEAM : captains
    USER }o--o{ TEAM : "member of"
    TOURNAMENT ||--|| LEADERBOARD : "has"
    TOURNAMENT ||--o| BRACKET : "has"
    BRACKET ||--o{ MATCH : contains
    TOURNAMENT ||--o{ MATCH : "has"
    TOURNAMENT ||--o{ PAYMENT : "for"
    USER ||--o{ PAYMENT : makes
    TEAM ||--o| PAYMENT : "entry paid by"
    USER ||--o{ EMAILOTP : "verifies via"
    ...
    TOURNAMENT {
        ObjectId _id
        string name
        string game "PUBG|FreeFire"
        string format "single-elimination|battle_royale_squad"
        number entryFee
        number prizePool
        number maxPlayers
        number maxTeams
    ...
    TEAM {
        ...
        ObjectId entryPayment FK
    }
    MATCH {
        ObjectId _id
        ObjectId tournament FK
        string kind "duel|br_lobby"
        number round
        number matchNumber
        ObjectId player1 FK
        ObjectId player2 FK
        ObjectId winner FK
        ObjectId winnerTeam FK
        string status
    }
    BRACKET {
        ObjectId _id
        ...
        number totalRounds
        bool isComplete
        ObjectId champion FK
        ObjectId championTeam FK
    }
    LEADERBOARD {
        ObjectId _id
        ObjectId tournament FK
    ...
    PAYMENT {
        ...
        string status
        string type "entry_fee|prize_payout"
        string transactionUuid UK
    }
    WALLET {
        ObjectId _id
        ObjectId user FK
...
classDiagram
    ...
    class Tournament {
        ...
        +Boolean autoGroup
        +Number lobbySize
        +ObjectId[] registeredPlayers
        +ObjectId[] registeredTeams
        +ObjectId[] soloPool
        +String status
    ...
    User "1" --> "*" Tournament : organises
    Tournament "1" --> "*" Team : registeredTeams
    Tournament "1" --> "1" Leaderboard
    Tournament "1" --> "*" Match
    Team "1" --> "*" User : members
    User "1" --> "1" Wallet
    Tournament "1" --> "*" Payment
```
...
flowchart LR
    ...
    O([Organiser])
    subgraph OGP
      UC1((Register via Email OTP))
      UC2((Verify OTP))
      UC3((Set Password & Role))
      UC4((Login))
...
```mermaid
flowchart TD
    S([Start]) --> E[User enters email + details]
    E --> G[System generates OTP, stores hash + expiry, sends via SMTP]
    G --> V[User enters OTP]
    V --> C{OTP valid & not expired?}
    C -- No --> R[Show error, allow retry / resend] --> V
    C -- Yes --> PW[User sets password & role]
    PW --> H[Hash password, mark verified, create Wallet, issue JWT]
    H --> D([End])
```
...
flowchart LR
    ...
    subgraph OGP
      ...
      L((List / Filter))
      RB((Register - bracket))
      RS((Create Squad + invite code))
      JS((Join Squad by code))
      JP((Join Solo - auto-group))
      ST((Start Tournament))
      VP((View Participants))
    end
    O --> C
    O --> ST
    P --> RB
...
```mermaid
flowchart TD
    S([Organiser clicks Start]) --> A{Auto-group enabled\n& solo pool non-empty?}
    A -- Yes --> B[Shuffle solo pool]
    B --> C[Chunk into squads of squadSize]
    C --> D[Create Team docs; add to registeredTeams; add to leaderboard]
    D --> E[Keep any lone leftover in pool - flagged]
    A -- No --> F
    E --> F{At least 2 squads?}
    F -- No --> G[Reject: need >= 2 squads]
    F -- Yes --> H[Split teams into lobbies of lobbySize]
    H --> I[Create one br_lobby Match per lobby]
    I --> J[Create Bracket referencing all lobbies]
    J --> K[status = ongoing]
    K --> D2([End])
```
...
sequenceDiagram
    ...
    participant O as Organiser Dashboard
    Player->>API: POST /matches/:id/stream (JWT, streamUrl)
    API->>API: protect middleware validates JWT
    API->>API: validate URL pattern
    alt invalid URL
        API-->>Player: 400 Bad Request
    else valid
        API->>DB: store URL on Match, set status=live
        DB-->>API: ok
        API-->>Player: 200 match updated
    end
    O->>API: GET /matches/tournament/:id
    API->>DB: fetch matches
    DB-->>API: matches with stream URLs
    API-->>O: render embedded stream panels
```
...
```mermaid
flowchart TD
    S([GET /leaderboard/global]) --> A[Load all Leaderboard docs, populate players]
    A --> B[For each entry: accumulate points, wins, matches by playerId]
    B --> C[Sort by points desc, then wins desc]
    C --> D[Assign ranks 1..n, apply limit]
    D --> E([Return ranked players])
```
...
sequenceDiagram
    actor Player
    participant API as Express API
    participant E as eSewa Gateway
    participant DB as MongoDB
    Player->>API: POST /payments/esewa/initiate (mode=solo)
    API->>API: assertCanJoinSolo (autoGroup, capacity, not already in)
    API->>DB: create pending Payment (metadata.soloPool=true)
    API-->>Player: signed payload + eSewa URL
    Player->>E: redirect, complete payment
    E->>API: GET /payments/esewa/success?data=...
    API->>E: verify via status API
    E-->>API: status COMPLETE
    API->>DB: mark Payment completed
    API->>DB: registerAfterPayment → add player to soloPool
    API-->>Player: redirect /payment/success
```
